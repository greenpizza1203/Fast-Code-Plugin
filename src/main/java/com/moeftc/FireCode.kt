...
open class FireCode : DefaultTask() {
    ...
    fun fire() {
        ...
        Socket(ip, 42069).getOutputStream().use { stream ->
            ...
            writeInt(stream, opModeBytes.size)
...
//        opModes.close()
    ...
    private fun writeInt(socket: OutputStream, num: Int) {
        socket.write(num ushr 24)
        socket.write(num ushr 16)
        socket.write(num ushr 8)
        socket.write(num)
    }
}